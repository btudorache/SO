...
static int write_file(SO_FILE *stream)
{
	...
	do {
		res =
		    write(stream->fd,
			  stream->file_buffer + stream->buffer_pointer - total,
			  total);
		total -= res;
...
SO_FILE *so_fopen(const char *pathname, const char *mode)
{
	...
	if (strncmp(mode, R_PLUS_MODE, strlen(R_PLUS_MODE)) == 0)
		fp_flags = O_RDWR;
	else if (strncmp(mode, R_MODE, strlen(R_MODE)) == 0)
		fp_flags = O_RDONLY;
	else if (strncmp(mode, W_PLUS_MODE, strlen(W_PLUS_MODE)) == 0)
		fp_flags = O_RDWR | O_CREAT | O_TRUNC;
	else if (strncmp(mode, W_MODE, strlen(W_MODE)) == 0)
		fp_flags = O_WRONLY | O_CREAT | O_TRUNC;
	else if (strncmp(mode, A_PLUS_MODE, strlen(A_PLUS_MODE)) == 0)
		fp_flags = O_RDWR | O_CREAT | O_APPEND;
	else if (strncmp(mode, A_MODE, strlen(A_MODE)) == 0)
...
int so_fclose(SO_FILE *stream)
{
	...
	free(stream);
	if (close_res == SO_EOF || flush_res == SO_EOF) return SO_EOF;
...
int so_fseek(SO_FILE *stream, long offset, int whence)
{
	...
	if (stream->file_pointer == SO_EOF) return SO_EOF;

	return 0;
}

long so_ftell(SO_FILE *stream) { return stream->file_pointer; }
...
int so_feof(SO_FILE *stream) { return stream->reached_feof; }

int so_ferror(SO_FILE *stream) { return stream->has_error; }
...
SO_FILE *so_popen(const char *command, const char *type)
{
	...
	case -1:
		...
		return NULL;
	case 0:
		if (strncmp(type, R_MODE, strlen(R_MODE)) == 0) {
			dup2(pipefd[1], STDOUT_FILENO);
			close(pipefd[0]);
		} else if (strncmp(type, W_MODE, strlen(W_MODE)) == 0) {
...
int so_pclose(SO_FILE *stream)
{
	...
	free(stream);
	if (close_res == SO_EOF || flush_res == SO_EOF || !WIFEXITED(status) ||
	    waitpid_res == SO_EOF) {
		return SO_EOF;
	}
